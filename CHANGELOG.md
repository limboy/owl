...
- Move the Add Folder button into the sidebar's toolbar beside the sidebar toggle, without it flashing next to the window title at launch
- Open a single video dropped on the window in a window of its own, listed afterwards under File → Open Recent
- Drop the dashed border and tint shown while dragging files over the window — the pointer's plus badge is the only cue now
